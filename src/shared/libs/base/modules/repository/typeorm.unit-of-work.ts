import { UnitOfWorkInterface } from '@libs/libs/base/modules/repository/unit-of-work.interface';
...
export class TypeormUnitOfWork implements UnitOfWorkInterface {
  ...
  async execute<T>(
    ...
    this.logger.debug(`[${correlationId}] [Starting transaction]`);
    ...
    this.logger.debug(`[${correlationId}] [Transaction committed]`);
  ...
  private async rollbackTransaction<T>(correlationId: string, error: Error) {
    ...
    try {
      ...
      this.logger.debug(
        `[${correlationId}] [Transaction rolled back] ${(error as Error).message}`,
      );
    } finally {
      await this.finish(correlationId);
    }
